...
Current homepage order (`app/page.js`): HeroSection → AboutSection → Experience → Skills → Projects → Education → ContactSection. (Blog removed 2026-08-07 — see §4 point 7.)
...
7. **Blog — removed 2026-08-07.** The dev.to integration (`devUsername: "Sujith-013"`) was checked directly (`curl https://dev.to/api/articles?username=Sujith-013` → `[]`) — the account has never posted, so the section rendered an empty grid on every load, which is a shipped-broken state, not an open design question anymore. Removed the section, `/blog` route, nav link, the dev.to fetch in `app/page.js`, and the now-unused `devUsername` field and `time-converter.js` helper. Full options considered and rationale in `docs/POLISH-AUDIT.md` — a static "writing/talks" placeholder (the IEPC presentation) remains a legitimate future idea, but is new content work, not something to build speculatively in a cleanup pass.
...
- ~~Not solving the dev.to `/blog` route's long-term fate in this pass~~ — resolved 2026-08-07: removed entirely (empty section, see §4 point 7).
