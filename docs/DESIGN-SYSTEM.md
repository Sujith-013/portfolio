...
- **Icon coverage for the skills grid** — most CAD/robotics tool names (SolidWorks, ANSYS, ROS 2, MuJoCo) still fall back to text-only tags (`utils/skill-image.js` only covers 10 of 25 skills). Not a Stage 3 concern; noted here so it isn't lost.
