...
**Completed 2026-08-05, fixed a shipped bug 2026-08-06.** `npm install gsap @gsap/react`. Built one shared primitive (`utils/hooks/use-section-reveal.js`) rather than the two originally separate 4.1/4.2 steps, because the planning change folding in mid-session (see `docs/PRD.md` §9-10 — the project showcase is now the primary deliverable) meant the reveal system had to handle image/figure entrances from the start, not just text — building it once, correctly, beat building a text-only version now and a second image-capable version later. `npx next build` clean.
...
**Shipped bug, found in live use 2026-08-06: every section below the hero rendered as an empty box, content never appearing at any scroll position.** Root cause, confirmed by reading GSAP's actual source (`node_modules/gsap/gsap-core.js`, `MatchMedia.prototype.add`): the original hook unconditionally hid every `[data-reveal]` element via a CSS class (`html.reveal-ready [data-reveal] { opacity: 0 }`) added on mount, then relied on `gsap.matchMedia({ reduceMotion: "..." }, callback)` to reveal them. `gsap.matchMedia`'s conditions-object form only invokes its callback when **at least one named condition's query currently matches**, or when a special `"all"` key is present — neither was true for any visitor *without* `prefers-reduced-motion` set (the overwhelming majority), so the reveal callback silently never ran, and content stayed hidden permanently regardless of scroll position. The multi-condition example in the `gsap-core` skill doc (`isDesktop`/`isMobile`/`reduceMotion` together) only appears to "always fire" because complementary breakpoint queries always have one match — that's a property of that specific example, not a general guarantee, and the single-condition case here fell straight into the gap.

**Fix:** rewrote the hook so nothing is ever hidden except in the same synchronous pass that immediately wires up its own reveal — no separate CSS-class mechanism, no dependency on a callback being reliably invoked. Reduced motion is now a plain `window.matchMedia("(prefers-reduced-motion: reduce)").matches` boolean check made *before* anything is hidden, not a condition a callback has to reach. The whole hide+reveal setup is wrapped in try/catch — if it throws for any reason, elements are force-set back to visible rather than left in whatever state the failure left them. Confirmed `useGSAP` uses `useLayoutEffect` on the client (checked `node_modules/@gsap/react/src/index.js` directly), so the hide-then-reveal setup still runs before the browser paints, preserving the original no-flash intent without needing the CSS class at all. Also hardened per the "check ScrollTrigger start/end" ask: added an `onRefresh` handler that force-reveals immediately if a trigger's calculated start point turns out to already be behind the current scroll position (e.g. a Lottie animation changed section height after initial calculation), plus a one-time `window.load` listener that calls `ScrollTrigger.refresh()` once all async content has settled.

**Verification, both times:** no browser automation tool was available in this session (or the original Stage 4 session). Verified via clean `npx next build`, SSR HTML inspection (all 28 `data-reveal` markers present and visible by default, `h1` clean, `reveal-ready` class confirmed absent from both HTML and compiled CSS post-fix), compiled-CSS inspection, and a clean dev-server log across multiple requests. **A live scroll-through plus a devtools `prefers-reduced-motion: reduce` emulation pass is still recommended before this ships** — per Stage 7.3's planned re-verification, and because this incident is direct proof the static checks alone didn't catch everything the first time.

**Re-checked 2026-08-06** after the Projects rebuild (58 `data-reveal` markers now, up from 28 — new project showcase/detail-route content): `use-section-reveal.js` confirmed byte-identical to the post-fix commit (untouched by today's work), `reveal-ready` class still absent from rendered HTML, `window.matchMedia("(prefers-reduced-motion: reduce)")` check still present and unchanged. The fix held.
...
**Partial progress 2026-08-06 — icon coverage resolved, marquee→grid rebuild still outstanding.** `utils/skill-image.js` now covers 14 of 25 skills with real, individually-verified brand icons: the 10 existing local SVGs plus `react-icons/si`'s `SiAnsys`/`SiArduino`/`SiRaspberrypi`/`SiNvidia` (checked against the package's type declarations directly — each is an unambiguous match for its skill). Two lookalikes were deliberately rejected: `SiYolo` (Simple Icons' "Yolo" is the Snap-affiliated anonymous-messaging app, not the YOLOv8 object-detection model — same name, wrong brand entirely) and `SiGoogle` (the plain "G" wordmark misrepresents Google Workspace, which has its own distinct product icon in reality). The remaining 11 skills (YOLOv8, ROS 2, Gazebo, Isaac Sim, MuJoCo, LiDAR/Depth, TensorRT, SolidWorks, Fusion 360, Simulink, Google Workspace) get one uniform `font-mono` monogram tile treatment — neutral `ink-*` tokens, no accent, applied identically rather than a one-off per skill. **Found during verification, not caused by it:** `react-fast-marquee` appears to render nothing at all server-side — not the old icons, not the new monograms, not even the wrapper markup — confirmed by diffing against the pre-existing committed version, which has the identical `<Marquee>` usage untouched by today's change. This means the whole Skills section has likely been JS-dependent-only since it was first built, invisible to no-JS/slow-JS visitors and to anything that only reads server-rendered HTML. Not fixed here (out of scope for an icon-coverage task) — worth checking explicitly in Stage 7's accessibility pass, and is itself one more argument for finally doing the marquee→static-grid replacement below rather than continuing to defer it.

**Resolved 2026-08-07 (Polish pass 1).** Replaced `<Marquee>` with a static grid, grouped by `skillsData`'s existing categories (a shape that's existed since Stage 1.4, previously flattened for the marquee) — same icon/monogram tile treatment, no new visual system. `react-fast-marquee` uninstalled from `package.json`; nothing else referenced it. Verified via curl'd SSR HTML: all 25 skill names, all 14 real icons, and all 11 monogram fallbacks are present in the server-rendered response — a no-JS visitor now sees the complete Skills section, not just a heading over blank space. Full findings in `docs/POLISH-AUDIT.md`.
...
### 5.5 Projects — re-scoped 2026-08-05, placeholder-first build landed 2026-08-06

**Progress 2026-08-06.** The format (Option 2: curated landing section + `/projects/[slug]` detail route) is fully built and live — with grey, dimension-labeled placeholder boxes standing in for every image/video slot, since no real assets exist yet. Two things changed alongside the build, both documented in full in `docs/PRD.md` §9.5:

- **The five featured projects changed.** The subject clarified there was no folder-mapping puzzle to solve (Step 0 in `docs/ASSET-CHECKLIST.md`, previously blocking) — the five domain folders (Aircraft Design, CubeSats and Satellites, Drone and Unmanned Aerial Vehicles, Rocket Design Technology, Rover) *are* the five projects, and they're personal/passion work, not the resume-verified professional projects (RobotX, Aliena PMA, Space Copy ISRU, Project New Dawn, DSTA CubeSat) originally slotted into this section. **Verified before removing anything:** every quantified result the professional projects carried already lived independently in `utils/data/experience.js`'s bullets and `utils/data/educations.js`'s achievements since Stage 1 (they were the source Stage 1.6 wrote the old project descriptions from) — nothing was deleted from the site, confirmed both in the data files and on the rendered page (RobotX's 2nd-place/95% mission assurance, the IEPC 2025 paper, DSTA's 1st place/400% pointing agility all still render, in Experience/Education).
- **Nothing about these five projects is verified content.** `docs/CONTENT-AUDIT.md` and the resume don't cover them at all. `projects-data.js` now holds clearly-marked `TODO:` strings for `nameFull`/`context`/`description`/`tools`/`result` per project — visible as literal text on the live page, not invented, not silently blank. `docs/ASSET-CHECKLIST.md` rewritten with the minimal content list needed per project plus the asset export steps.

**What got built:** deleted the fake-terminal `project-card.jsx` entirely (traffic-light dots, `const project = {...}` block — the exact template-leftover conceit flagged back in the original architecture audit, now gone along with the professional-project content it was styled around). New `ProjectShowcase` component gives each project its own full landing-page section (replacing the old 5-card sticky stack — the `.sticky-card`/`#sticky-card-N` CSS is deleted too, since stacking full-viewport sections doesn't make sense at this size and wasn't part of the chosen direction). New `app/projects/[slug]/page.js` — a real static route per project (`generateStaticParams`, verified all 5 paths build) with a captioned gallery grouped by category (Design & CAD / Simulation & Analysis / Results & Data / Video / Code). New `AssetPlaceholder`/`CodePlaceholder` components (`app/components/helper/asset-placeholder.jsx`) render dashed-border grey boxes at the exact intended export dimensions with the pixel size written on them — deliberately not styled to look like real content, so the format can be judged and approved before any real export happens. Motion: no new system — `ProjectShowcase` and `ProjectDetailView` each call the existing `useSectionReveal` hook on their own scope (a full project section is now sized/weighted like any other page section, so it gets its own reveal call, same as Hero/About/etc.), with `data-reveal="figure"` on every placeholder box exactly as the primitive was designed to support back in Stage 4.

**Video added to the pipeline** (`docs/PRD.md` §10.5, `docs/ASSET-CHECKLIST.md`): MP4/H.264, capped at 1080p, ≤8MB/clip, poster frames required, click-to-play by default (not autoplay), `preload="none"`, and reduced-motion never autoplays under any circumstance — the same discipline Stage 4's reveal system already applies to scroll animation, applied to a second motion source. Code snippets are a fifth gallery category, but text in the manifest, not a file — one element among the others, never the page's frame.

**Progress 2026-08-07 (Polish pass 3) — the real-asset rendering path now exists, ahead of any asset landing.** `AssetPlaceholder`/`CodePlaceholder` only ever rendered the grey placeholder box — there was no code path that would ever show a real image/video even once one existed, since nothing read `asset.placeholder`/`asset.file` at all. Built `ProjectAsset` (same file) to close that gap before it could be forgotten under time pressure once real assets start arriving: renders the placeholder for any asset still `placeholder: true` (everything today, unchanged), or a real `next/image`/`<video>` once an entry is wired to `placeholder: false` — so wiring in a project per `docs/ASSET-CHECKLIST.md` becomes a pure data edit. Implements `docs/PRD.md` §10.4-10.5's loading-strategy rules directly: homepage showcase heroes stay lazy (never `priority`), the detail-page hero gets `priority` (first-viewport, opted-into content), gallery images get a real `sizes` attribute matching the actual grid layout, and video is `preload="none"` with a poster and no autoplay/loop — click-to-play by default, which also means the reduced-motion rule holds structurally rather than needing its own conditional. `CodePlaceholder` got the same treatment for real code snippets. Validated against synthetic (non-project) test images through Next's real image optimizer, not just read from the PRD — see `docs/POLISH-AUDIT.md`'s Polish pass 3 §2 for the numbers. `projectsData` itself only gained a `placeholder: true` flag on each `hero` object (schema, matching the shape gallery assets already had) — no content changed.
...
### 5.7 Blog section decision — ✅ DONE (removed, 2026-08-07)

**Resolved 2026-08-07 (Polish pass 1).** Checked the dev.to fetch directly (`curl https://dev.to/api/articles?username=Sujith-013` → `[]`) — the account has never posted, so the section and the `/blog` route were both shipping an empty grid, not just carrying an open design question. Removed cleanly: `app/page.js`'s dev.to fetch and `<Blog>` render, `app/blog/page.js`, `app/components/homepage/blog/` (both files), the "BLOGS" nav link in `app/components/navbar.jsx`, the now-unused `devUsername` field in `personal-data.js`, and `utils/time-converter.js` (its only consumer was the deleted blog card). No package was blog-specific, so no dependency removal needed here. Full options considered (repoint, static placeholder, remove) and rationale in `docs/POLISH-AUDIT.md` — removal was the only option that didn't require either fabricating an account to point at or writing new, unverified "talks" content in what was meant to be a cleanup pass.
...
**Checked 2026-08-07 (Polish pass 1).** `EMAIL_ADDRESS`/`GMAIL_PASSKEY` are still unset, so the 503 path (from Stage 2.3) was re-verified live rather than just read: `POST /api/contact` returns a real `503` with the honest "not configured yet" message, and `contact-form.jsx`'s toast surfaces it — no silent failure, no false success. One real gap fixed regardless of the keep/replace question below: the toast only had real text for API-reported errors (`error.response.data.message`); a network failure or timeout had no message to read and would have toasted blank, which reads as a silent failure even though the code technically tried to report one. Added a fallback string so every failure path shows real text. **Not decided here, per instruction:** whether to keep the form (works once two env vars are set, better UX, more code to keep correct) or replace with a `mailto:` link (zero config, cannot silently rot, worse UX on visitors without a configured mail client) — both sides argued in full in `docs/POLISH-AUDIT.md`, recommendation is to keep the form once the env vars are set, awaiting the subject's call.
...
**Partial progress 2026-08-07 (Polish pass 3) — bundle size measured and a real regression found/fixed, ahead of the formal Lighthouse run.** Full detail and the before/after numbers in `docs/POLISH-AUDIT.md`. Summary: measured the homepage's actual production JS payload (`next build && next start`, summed every chunk in the initial `<script>` tags — same method as Stage 4's GSAP measurement) and found it driven substantially by two Lottie animation JSON files (`code.json` 84KB, `study.json` 475KB) that were statically imported into their component's own chunk — the `lottie-react` *library* was already correctly lazy-loaded (`next/dynamic({ ssr: false })`), but the *data* wasn't, so 564KB of raw keyframe JSON shipped to every homepage visitor regardless of whether they scrolled to Education. Fixed by fetching the JSON at runtime instead of importing it; homepage payload dropped from 1360KB/379KB (raw/gzip) to 815KB/263KB — a 31% gzip reduction. Also deleted 7 more unimported Lottie files (~1.5MB) found dead in `app/assets/lottie/` while investigating. This wasn't caused by anything in this build's own work — it predates Stage 4-7 entirely — but it's exactly the kind of thing 8.2's bundle check is meant to catch, so it's logged here rather than left for a later pass to rediscover.

Also modeled §10.4's per-image performance budget empirically (synthetic test images at the specced dimensions run through Next's real optimizer, including a deliberately adversarial gradient+noise case) rather than trusting the PRD's numbers by formula alone — confirmed the budget holds with 2-14x headroom even in the worst-case test, so no renegotiation is needed once real assets land. See `docs/POLISH-AUDIT.md`'s Polish pass 3 §2 for the full table.
...
**Partial progress 2026-08-07 (Polish pass 3) — the hero's mobile 20-second test was failing, now fixed.** The hero's two-column grid used `order-*` utilities to deliberately show the decorative fake-terminal code panel *before* the name/headline on mobile — and that panel's skills line grew to all 25 real skills (from a handful of hardcoded ones) without anyone revisiting how it stacks below `lg`, so a mobile visitor's first several hundred pixels of scroll had become a wrapped, multi-line skills dump, with the actual `<h1>` pushed below the fold. Fixed by deleting the `order-*` classes entirely — DOM order already had text-then-panel, so this restores natural order on mobile with zero effect on desktop's appearance (order classes were redundant with DOM order at `lg`). Full responsive pass (five 380px-viewport project-format checks, About's similar-looking but non-problematic use of the same class pattern, horizontal-overflow grep) in `docs/POLISH-AUDIT.md`'s Polish pass 3 §1. Still owed: an actual hands-on resize/DevTools pass — this was verified via SSR HTML/class inspection, no browser automation tool available in this environment (same caveat as Stage 4).
