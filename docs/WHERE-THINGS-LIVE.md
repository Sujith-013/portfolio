...
**Font loading:** `app/layout.js` — three `next/font/google` calls (`IBM_Plex_Sans`, `Roboto_Slab`, `IBM_Plex_Mono`), each exposing a CSS variable consumed by `theme.css`. Plex Sans and Roboto Slab load `weight: "variable"`, not a static array — see `docs/DESIGN-SYSTEM.md` "Typography" for why (a real next/font+Turbopack bug collapses discrete static weights on variable-capable families).
**Scale/roles:** `app/css/theme.css`'s `@theme` block — `--font-sans` (IBM Plex Sans, body text, the default), `--font-display` (Roboto Slab, headings/nav logo/section eyebrows/project titles), `--font-mono` (IBM Plex Mono, dates/tags/spec labels/code). `--text-display: 2.75rem` is the one custom type-scale token (fills a real gap between Tailwind's default `text-4xl`/`text-5xl`), used on the hero `<h1>`.
...
- **`app/components/helper/card.jsx`** — the Experience/Education card shell. Used to be `card.scss` + `glow-card.jsx`, a mouse-tracked conic-gradient hover glow with its own hand-written CSS file; removed during the color audit (see `docs/DESIGN-SYSTEM.md` "Audit: gradients, glow, blur, decorative stripes") as a glassmorphism-template pattern independent of hue. Now a plain server component, no client JS, no dedicated CSS file — the hover state is one Tailwind class (`hover:border-accent`). There is currently no hand-written CSS anywhere in the repo outside `globals.scss`'s two base rules.
...
- **No arbitrary Tailwind bracket values** (`p-[13px]`, `text-[#fff]`, etc.) for anything a token should cover. The one legitimate class of exception already in the codebase is one-off sub-token pixel dimensions for a plain neutral hairline (e.g. `h-[2px]` on the `bg-border-strong` flanking rules around section eyebrow labels) — not colors, not spacing that should follow the `my-12 lg:my-24` rhythm.
...
### Nav links
**File:** `app/components/navbar.jsx` — the `NAV_LINKS` array near the top (`{ href, label }` pairs, ordered to match the page's actual scroll order — About, Experience, Skills, Projects, Education). Both the desktop nav and the mobile hamburger menu render from this same array — no second list to keep in sync. Anchor links (`/#about` etc.) must match the target section's `id` — that `id` is set on the section's own wrapping `<div>` inside its own component file (e.g. `id="about"` lives in `app/components/homepage/about/index.jsx:15`, not in `app/page.js` — `page.js` only imports and orders the section components, it doesn't carry their ids).
...
**Client components** (`"use client"` at the top of the file): everything under `app/components/homepage/*`, `app/components/navbar.jsx`, and one helper (`scroll-to-top.jsx`). These can use hooks and browser APIs, but **cannot** be `async` functions or export `generateStaticParams`/`generateMetadata`. Every homepage section is a client component because every one of them calls `useSectionReveal` (needs `useRef` + GSAP, both client-only). `app/components/helper/card.jsx` is a plain server component (no hooks, no browser APIs) — it used to be the client-only `glow-card.jsx` before the color audit removed its mouse-tracking; see `docs/DESIGN-SYSTEM.md`. (`animation-lottie.jsx` — the third former client helper — is gone; see `docs/DESIGN-SYSTEM.md` "Audit: AI-template visual clichés" for why the two Lottie illustrations it rendered were removed.)
...
**Every open yes/no decision (contact form, resume hosting, GTM, etc.) is tracked in one place: `docs/DECISIONS.md`.** It isn't duplicated here on purpose — this exact section used to list the contact-form decision as "undecided" after it had already been resolved in a later pass, because two documents were tracking the same status independently. Check `docs/DECISIONS.md` for current decision status; what follows here is unfinished verification and known-intentional state, not decisions.

1. **The five projects' content is entirely placeholder.** Every `nameFull`/`context`/`description`/`tools`/`result` in `utils/data/projects-data.js` is a literal `TODO:` string, and every asset is `placeholder: true`. This is intentional, not a bug — see [Adding assets](#adding-assets), `docs/CONTENT-TEMPLATE.md`, and `docs/ASSET-CHECKLIST.md` for exactly what's needed per project.
2. **A hands-on browser/DevTools pass is still owed** for both responsive layout and keyboard navigation. Every verification across all polish passes was done via SSR HTML inspection, computed class/aspect-ratio reasoning, and live-but-scripted checks — no interactive browser automation tool is available in this environment. Worth an actual resize-and-tab-through pass before shipping real assets.
3. **Formal Lighthouse run (`docs/BUILD-PLAN.md` Stage 8.1) hasn't happened** — bundle size and per-image budgets were verified manually and empirically (see `docs/POLISH-AUDIT.md` Polish pass 3 §2), but an actual Lighthouse score against a deployed preview is still outstanding.
4. **Minor: a vestigial `video: true` field.** `projects-data.js`'s `demoAssets()` helper sets `{ video: true }` on its placeholder video entry, but nothing reads that field — `ProjectAsset` and `AssetPlaceholder` both branch on `category === "video"` instead. Still present, still harmless, safe to remove next time that helper is touched, not worth a standalone pass.
